...
export default async function handler(req, res) {
  ...
  try {
    await sql`DROP TABLE IF EXISTS inscricoes CASCADE;`;
    await sql`DROP TABLE IF EXISTS eletivas CASCADE;`;

    await sql`
      CREATE TABLE eletivas (
        id VARCHAR(50) PRIMARY KEY,
        title VARCHAR(100) NOT NULL,
        max_limit INTEGER DEFAULT 28,
    ...
    if (eletivasCount[0]?.count === '0') {
      ...
      for (const [id, title, max_limit] of defaultEletivas) {
        await sql`
          INSERT INTO eletivas (id, title, max_limit)
          VALUES (${id}, ${title}, ${max_limit})
